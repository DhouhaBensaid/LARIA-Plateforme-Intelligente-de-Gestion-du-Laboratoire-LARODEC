...
import { Users, FileText, GraduationCap, Calendar, Handshake, FileBarChart, Building2, LogOut, Home, User, ChevronLeft, ChevronRight, ArrowLeftRight } from "lucide-react";
...
export function Layout() {
  ...
  const displayName   = session?.user.nom && session?.user.prenom
    ? `${prefix} ${session.user.prenom} ${session.user.nom}`.trim()
    : isAdminUser ? "Admin LARODEC" : session?.user.email ?? "...";
      ...
      <aside className={`${collapsed ? "w-[68px]" : "w-64"} flex-shrink-0 h-screen bg-white border-r border-slate-100 flex flex-col transition-all duration-300 shadow-sm`}>
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          {menuItems.map(({ icon: Icon, label, path, color }) => {
            const c = C[color];
            const isActive = (path === "/admin" || path === "/chercheur")
          ...
          <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-100 ${collapsed ? "justify-center" : ""}`}>
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-400 truncate">{displayedRole}</p>
              </div>
            )}
          </div>
